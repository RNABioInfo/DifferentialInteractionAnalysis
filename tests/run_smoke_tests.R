...
params <- list(
  ...
  annotation = list(
    enabled = TRUE,
    gff_file = file.path(fixture_dir, "synthetic_annotations.gff3"),
...
balanced_params$out_dir <- tempfile("dia_smoke_batch_")
...
confounded_params$out_dir <- tempfile("dia_smoke_confounded_")
...
off_params$out_dir <- tempfile("dia_smoke_batch_off_")
...
message("Smoke tests passed. Output directory: ", out_dir)
